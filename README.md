...
**Note**: The Large Language Model (LLM) never directly accesses raw log data, ensuring data privacy. Instead, it operates on schema and ontology context provided by the Knowledge Graph. The orchestrator manages complex planning and verification loops.
...
The `outputs/` folder serves as a centralized location for all generated artifacts and results from the threat hunting process. This includes, but is not limited to:

*   **Evaluation Reports**: Detailed reports generated by the `eval-runner` service, summarizing the performance and findings of automated hunts. These might be in formats like Markdown (`.md`) or JSON (`.json`).
*   **Hunt Logs**: Raw logs and verbose outputs from agent execution, useful for debugging and understanding the agent's step-by-step reasoning.
*   **Generated Data**: Any intermediate or final data generated during the hunt, such as refined queries, extracted entities, or aggregated results.

This folder is essential for reviewing the outcomes of hunts, tracking evaluation progress, and performing post-analysis.
